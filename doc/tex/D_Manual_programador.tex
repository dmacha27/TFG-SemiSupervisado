...
desde la página de Git SCM (\textit{Source code management})\footnote{Git para
Windows: \url{https://git-scm.com/download/win}}.

Pycharm se puede descargar tanto para Windows como para Linux en la página
oficial de JetBrains\footnote{Pycharm:
...
este directorio como raíz. La idea es que cada fichero <<\texttt{.py}>>,
homónimo al algoritmo, contenga la definición de un objeto que encapsule el
...
cómo sea el algoritmo. Por ejemplo, en el caso de \textit{Democratic
Co-Learning} se hacía imprescindible añadir estadísticas específicas para cada
clasificador que encapsula y por tanto, retornaba más elementos.
...
terminan por <<\texttt{\_routes.py}>> donde se definen. Si se añade una
categoría nueva se debe crear un Blueprint que la identifique dentro de su
fichero y después debe ser registrado en la aplicación (en
...
\textbf{Modelos de la base de datos}: En <<\texttt{models.py}>> se tienen
definidas todas las entidades que maneja la aplicación: \texttt{Users},
\texttt{Datasets} y \texttt{Runs}. Si se desea añadir alguna, primero se debe
crear aquí la definición de la misma. Para que la aplicación sepa las entidades
que debe manejar, también es recomendable importarla en la creación de la
aplicación.
...
correspondiente en <<\texttt{forms.py}>> con los campos necesarios. Hasta el
momento, los algoritmos que hay implementados tienen una parte en común que está
definida en la clase <<\texttt{FormConfiguracionBase}>> y en principio, si se
añade uno nuevo, deberá extender dicha clase.
...
El contenido dinámico debe ser creado mediante Javascript, de forma
<<\textit{vanilla}>> (sin utilizar \textit{frameworks}). 
...
Con <<\textit{step}>> se debe tener en cuenta la posibilidad de permitir números
enteros (al introducir 1) o números decimales (al incluir un flotante, por
ejemplo, 0.01).
...
Además de esto, hay tres zonas en la lógica de los \textit{endpoints} que deben
...
con el <<\textit{timestamp}>> concatenado al nombre del fichero introducido.
...
sub-carpeta distinta a la de los usuarios registrados. Con
<<\texttt{current\_user}>> de \texttt{Flask-Login} se puede conocer si se trata
de usuarios anónimos o registrados. En función de esto, la aplicación tiene
almacenada una ruta para los ficheros anónimos
(<<\texttt{CARPETA\_DATASETS\_ANONIMOS}>>) y otra para los registrados
(<<\texttt{CARPETA\_DATASETS\_REGISTRADOS}>>).
...
embebida directamente en <<\texttt{<script>}\footnote{Etiqueta HTML}>>. Al
incluirse de esta forma, Babel sí puede capturar esas funciones y al acceder a
la función retornar el texto adecuado. 
...
<<\texttt{@app.context\_processor}>>). A partir de aquí, cuando existe un texto
generado en JavaScript que necesita ser traducido, se define un diccionario que
contiene en los valores las palabras deseadas. Este diccionario realmente será
un diccionario de diccionarios, el primer nivel tendrá el idioma.
...
//Windows
$ MD runs
$ MD datasets\anonimos # el directorio datasets ya está creado
...
//Linux
$ mkdir runs
$ mkdir datasets/anonimos # el directorio datasets ya está creado
...
Una vez que se han extraído los textos, se han de añadir las traducciones (a
mano). Todos los textos envueltos por las funciones antes comentadas actúan como
identificadores. 

En \texttt{translations/es/LC\_MESSAGES/messages.po} se tienen esos
identificadores junto con la traducción (estará vacía si no se ha introducido).
En ese fichero se incluirán las traducciones.

El siguiente paso es la compilación de estas traducciones para que
\texttt{Babel} pueda sustituir los textos dinámicamente.
...
capa consiste en establecer un servidor WSGI (\textit{Web Server Gateway
Interface}) que dictará la comunicación entre el servidor web (a discreción del
programador) y Flask. 

En el proyecto actual este servidor ha sido generado por Gunicorn (\textit{green
unicorn}) y su uso es extremadamente simple\footnote{Se encuentra entre los
requisitos de bibliotecas del proyecto}.
...
(en un entorno Linux) y debe ser tomado como un ejemplo. Gunicorn no puede ser
usado en sistemas Windows (el servidor). Una alternativa muy usada para estos
sistemas es waitress\footnote{waitress:
...
IDE>> en el navegador Google Chrome (incorpora funcionalidades que no existen en
otros) y activar en la configuración de la extensión de Selenium IDE la opción
<<Permitir acceso a URL de archivo>>. Esta opción se utiliza para poder subir
archivos. Es más, también se deben configurar las rutas de los ficheros en los
pasos en los que estos se cargan (ver descripción de los casos de prueba).
...
A continuación se presentan todos los casos de prueba que validan el correcto
funcionamiento de las partes más importantes de la aplicación.
...
\begin{itemize}
    ...
    \item \textbf{Correo electrónico}: \texttt{prueba@gmail.com}
...
Acceder a `\texttt{/?lang=es}'                       &                                                                                                         & Encontrarse en la página principal                                   & Encontrarse en la página principal                                   & Éxito                            \\ \hline
...
Hacer clic en el botón de <<Inglés>>                   &                        & El subtítulo principal es <<\textit{Final Degree Project on semi-supervised algorithms and their visualization}>>  & El subtítulo principal es <<\textit{Final Degree Project on semi-supervised algorithms and their visualization}>>  & Éxito                            \\ \hline
\end{tabular}
...
Acceder a `\texttt{/?lang=es}'                          &                        & Encontrarse en la página principal                                   & Encontrarse en la página principal                                   & Éxito  \\ \hline
Hacer clic en el nombre de usuario <<Prueba>> y en el botón de <<Perfil>>      &                        & Encontrarse en la página del perfil de usuario                                 & Encontrarse en la página del perfil de usuario                                & Éxito  \\ \hline 
Introducir datos de usuario (contraseña errónea)                           & Nombre: Prueba, Email: pruebacambiado@gmail.com, Contraseña actual: 1234567899                  & Datos introducidos                                                   & Datos introducidos                                                   & Éxito                            \\ \hline
...
\rowcolor{gray!25}
\textbf{Paso}   & \textbf{Entrada} & \textbf{Resultado esperado} & \textbf{Resultado obtenido} & \textbf{Estado} \\ \hline
Acceder a `\texttt{/admin?lang=es}'                                          &                        & Encontrarse en la página de error                                  & Encontrarse en la página de error                                    & Éxito  \\ \hline
...
\rowcolor{gray!25}
\textbf{Paso}   & \textbf{Entrada} & \textbf{Resultado esperado} & \textbf{Resultado obtenido} & \textbf{Estado} \\ \hline
Acceder a `\texttt{/admin?lang=es}'                         &                        & Encontrarse en la página de inicio de sesión                                 & Encontrarse en la página de inicio de sesión                                     & Éxito  \\ \hline
...
\rowcolor{gray!25}
\textbf{Paso}   & \textbf{Entrada} & \textbf{Resultado esperado} & \textbf{Resultado obtenido} & \textbf{Estado} \\ \hline
Acceder a `\texttt{/}'                                         &                        & Encontrarse en la página principal                                   & Encontrarse en la página principal                           & Éxito  \\ \hline
Hacer clic en <<Self-Training>>                       &                        & Encontrarse en la página de subida                                   & Encontrarse en la página de subida                           & Éxito                            \\ \hline
Subir el conjunto de datos iris                       & Fichero <<\texttt{iris.arff}>>  & Porcentaje de subida al 100\%                                        & Porcentaje de subida al 100\%                                & Éxito                            \\ \hline
...
Comprobar email actualizado del usuario <<Prueba>> en la tabla de usuarios      &                       &  El email que aparece es <<pruebaadministrador@gmail.com>>     &  El email que aparece es <<pruebaadministrador@gmail.com>>  & Éxito \\  \hline
\end{tabular}
