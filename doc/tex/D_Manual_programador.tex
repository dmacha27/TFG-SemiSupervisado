...
que hay implementados tienen una parte en común que está definida en la clase
<<\texttt{FormConfiguracionBase}>> y en prinicpio, si se añade uno nuevo, deberá
extender dicha clase.
...
extender. También se debe la nomenclatura utilizada para el resto de algoritmos
(están nombrados de forma intuitiva).

\textbf{Ficheros estáticos (static)}: El contenido dinámico debe ser creado
mediante Javascript, de forma <<vanilla>> (sin utilizar frameworks). Es
completamente necesario manetener la organización actual de las carpetas, las
funciones que se añadan deberán incluirse en la carpeta de su contexto. Todos
los métodos se encuentran documentados.
...
JSON <<parametros.json>>. Hasta el momento del desarrollo solo existen dos tipos
de entradas, los selectores y los numéricos.
...
ya que una vez leido este fichero, JavaScript lo recorre creando tantas opciones
en los selectores como clasificadores haya.

Además de esto, hay tres zonas en la lógica de los \texttt{endpoints} que debe
ser modificados si se añade un algoritmo. En <<\texttt{data\_routes.py}>>,
...
\textbf{Conjunto de datos (datasets)}: Los conjuntos de datos de los usuarios
(vinculados a sus sesiones) se almacenan localmente en la aplicación con el
<<Timestamp>> concatenado el nombre del fichero introducido.
...
como Python si fuera necesario). El proceso de extracción y compilación de las
traducciones está explicado en la siguiente sección.
...
extracción sin incluir <<\texttt{lazy\_gettext}>> (por ejemplo) y a continuación
se actualizan las traducciones, se perderán aquellas que tenían
<<\texttt{lazy\_gettext}>>.
